...
```{r}
task_b <- function(main, sub, method = "pmm", maxit = 5, m = 5){
  main["X"] <- NA
  ...
  ret <- list("beta_0" = b_res_sum$estimate[1],
              "beta_0_sigma" = b_res_sum$std.error[1],
              "beta_1" = b_res_sum$estimate[2],
              "beta_1_sigma" = b_res_sum$std.error[2],
              "beta_1_confint" = c(b_res_sum$`2.5 %`[2], b_res_sum$`97.5 %`[2]))
...
```{r}
task_c_basic <- function(main, sub, method = "pmm", maxit = 5, m = 5, exclude_y_for_x = F){
  main["X"] <- NA
  sub["Y"] <- NA
  ...
  # Predictor matrix
  c_pred_mat <- matrix(rep(0, 9), ncol = 3)
  rownames(c_pred_mat) <- colnames(c_pred_mat) <- c("X", "Y", "W")
  ...
  ret <- list("beta_0" = c_res_sum$estimate[1],
              "beta_0_sigma" = c_res_sum$std.error[1],
              "beta_1" = c_res_sum$estimate[2],
              "beta_1_sigma" = c_res_sum$std.error[2],
              "beta_1_confint" = c(c_res_sum$`2.5 %`[2], c_res_sum$`97.5 %`[2]))
...
## Method Raghunathan talked about in his book

Regunathan describes a method using sampling in his book. We tried to replicate it, but it did not return usable results in our case.
...
```{r}
task_c_sequential <- function(main, sub, method = "pmm", maxit = 5, m = 5, thresh = 1e-09, max_iter = 100, print = T){
  ...
  while (n_runs <= max_iter & convergence_val > thresh){
    ...
    beta_0_archive <- append(beta_0_archive, current_weights[1])
    beta_1_archive <- append(beta_1_archive, current_weights[2])
  ...
  ret <- list("beta_0" = c_sequential_res_sum$estimate[1],
              "beta_0_sigma" = c_sequential_res_sum$std.error[1],
              "beta_1" = c_sequential_res_sum$estimate[2],
              "beta_1_sigma" = c_sequential_res_sum$std.error[2],
              "beta_1_confint" = c(c_sequential_res_sum$`2.5 %`[2], c_sequential_res_sum$`97.5 %`[2]))
...
```{r}
task_d_basic <- function(main, sub, method = "pmm", maxit = 5, m = 5){
  main["X"] <- NA
  sub[1:50, "Y"] <- NA
  sub[51:100, "W"] <- NA
  ...
  ret <- list("beta_0" = d_res_sum$estimate[1],
              "beta_0_sigma" = d_res_sum$std.error[1],
              "beta_1" = d_res_sum$estimate[2],
              "beta_1_sigma" = d_res_sum$std.error[2],
              "beta_1_confint" = c(d_res_sum$`2.5 %`[2], d_res_sum$`97.5 %`[2]))
...
```{r}
task_d_sequential <- function(main, sub, method = "pmm", maxit = 5, m = 5){
  main["X"] <- NA
  sub[1:50, "Y"] <- NA
  sub[51:100, "W"] <- NA
  ...
  pred_map_step_one <- matrix(rep(0, 9), ncol = 3)
  rownames(pred_map_step_one) <- colnames(pred_map_step_one) <- c("X", "Y", "W")
  ...
  pred_map_step_two <- matrix(rep(0, 9), ncol = 3)
  rownames(pred_map_step_two) <- colnames(pred_map_step_two) <- c("X", "Y", "W")
  ...
  pred_map_step_three <- matrix(rep(0, 9), ncol = 3)
  rownames(pred_map_step_three) <- colnames(pred_map_step_three) <- c("X", "Y", "W")
